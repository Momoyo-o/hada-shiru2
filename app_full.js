...
// 🔐 パスワード認証（APIキーは不要）
const APP_PASSWORD = (function initAuth() {
    const savedPassword = sessionStorage.getItem('hadashiru_auth');
    ...
    if (!password) {
        ...
        throw new Error('認証キャンセル');
    }
    
    sessionStorage.setItem('hadashiru_auth', password);
    return password;
})();

// 🌐 Gemini API呼び出し（Netlify Functions経由）
async function callGeminiAPI(requestBody) {
    const endpoint = window.location.hostname === 'localhost' || window.location.hostname === '127.0.0.1'
        ? 'http://localhost:8888/.netlify/functions/gemini-proxy'  // ローカル開発時
        : '/.netlify/functions/gemini-proxy';  // 本番環境
    ...
    if (!response.ok) {
        if (response.status === 401) {
            ...
            sessionStorage.removeItem('hadashiru_auth');
            window.location.reload();
...
読み取った文字をそのまま、カンマ区切りで出力してください。`;
    ...
    // callGeminiAPIは成功時のみデータを返すため、エラーチェック不要
    const data = response;
